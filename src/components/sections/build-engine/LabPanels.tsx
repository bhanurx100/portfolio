  ...
  return (
    <div className="max-w-xl mx-auto w-full space-y-4">
      <motion.div
        ...
        className={`flex items-center gap-2 px-4 py-3.5 rounded-xl border ${
      ...
      >
        <Sparkles className={`w-4.5 h-4.5 shrink-0 ${isDark ? 'text-[var(--accent)]' : 'text-[var(--accent)]'}`} style={{ width: 18, height: 18 }} />
        <input
          type="text"
          value={value}
    ...
    <div className={`${panel(isDark)} p-3 space-y-3`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {simState === 'idle' && (
            <button onClick={onStart} className={`px-3 py-1 rounded-lg text-xs font-semibold ${isDark ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'}`}>
              Run system
            </button>
          )}
          {simState === 'running' && (
